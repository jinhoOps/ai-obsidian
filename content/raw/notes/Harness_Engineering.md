...
내가 이해한 바를 곁들여서 정리하면,
문제정의 라는 출발지에서 해결이라는 목적지까지 가기위한 과정이 문제해결력으로 불리고 있음. 그걸 AI가 할수있게 원칙들을 정의하고 상황에 맞게 방향을 잡아주는 것을 하네스 라고 부르고, 결국 단순화해서 보면 출발! 명령하면 도착! 답변 할때까지 루프를 돌수있게, 그 루프를 효율적으로 만드는게 하네스 엔지니어링 인것같음.
즉, 아래 내용 중 앤트로픽이 더 넓게 단어가 가지는 뜻을 잘 나타냈다고 보임.
openai의 엔지니어 관점은 기존 skills+agents 확장한 개념과 토큰 사용 최적화 등을 묶어서 말한거같음.
