...
## [2026-07-08] lint | 위키 건강 검진 및 자동/수동 잔재 정비 완료
- **건강 점수**: 8 → 90 (🟢 건강) *예외로 인정한 draft: true 규칙 문서(ai-rules) 제외 시 실질 건강도 100점*
- **발견 / 수정**: 깨진링크 3/3, Frontmatter 2/2, 인덱스 불일치 3/3, 고아 페이지 1/1, 유령 개념 3/3
- **작업 상세**:
    - 불필요하게 `content/wiki/` 하위에 잘못 생성되어 빌드를 방해하던 `graphify-out/` 잔재 폴더 삭제 완료.
...
## [2026-07-08] ingest | Getting started with loops 아티클 인제스트 완료
- **인제스트 내용**:
    - [[getting-started-with-loops]] : Claude Code 팀의 에이전트 루프 공식 분류 및 최적화 방법론 요약.
    - [[agentic-loop]] : 에이전트의 자율적 4단계 루프(Turn-based, Goal-based, Time-based, Proactive)와 자가 검증 개념 신규 생성.
    - **지식 연결**: 사용자 맥락("새로운 방법론 공부 및 AI 지식 공유 준비")을 반영하여, 향후 지식 공유 세션에서 활용 가능한 개념 정리 및 제언 추가.
- **업데이트된 페이지**:
    - [[index.md]] (System) : 소스 요약 및 개념 카테고리에 신규 페이지 등록 및 통계(총 페이지 65개, 소스 수 31개) 갱신.
    - [[claude-code]] (Entity) : 에이전트 루프 공식 지원 기능 및 소스 출처 업데이트.
- **잔여 문제**: 없음.

---

## [2026-06-08] ingest | OpenHuman 아티클 인제스트 완료
- **인제스트 내용**:
    - [[openhuman-article]] : '나를 아는 AI 비서'를 오픈소스로 만들 수 있을까 아티클 요약.
    - [[openhuman]] : 기억과 로컬 데이터 중심의 오픈소스 AI 개인 비서 프로젝트 엔티티 추가.
    - [[memory-tree]] : 비정형 데이터를 계층적 요약 트리 구조로 관리하는 로컬 지식 백엔드 개념 추가.
    - [[token-compression]] : LLM 토큰 소모량 및 지연 시간 최적화를 위한 텍스트 전처리 개념 추가.
    - **지식 연결**: 사용자 맥락("써보고 프로젝트 도입 검토. 테스트 잘되면 ai-obsidian 방식 변경할 수도 있음")을 반영하여 연계 제언을 남김.
- **업데이트된 페이지**:
    - [[index.md]] (System) : 엔티티, 개념, 소스 요약 카테고리에 신규 추가 및 통계(총 페이지 수 63개, 총 소스 수 30개) 갱신.
    - [[index.md]] (Raw Articles) : raw 아티클 목록에 OpenHuman 추가.
- **잔여 문제**: 없음.

---

## [2026-06-08] synthesis | 6월 첫째 주 눈여겨봐야 할 20대 AI 소식 종합
- **작업 내용**:
    - [[top-20-ai-news-2026-06]] : 2026년 6월 첫째 주 AI 업계 스레드 뉴스에서 선별한 20대 소식 요약 및 트렌드 분석 문서 신규 생성.
    - **지식 연결**: 메타 보안사고와 [[harness-engineering]]의 관계성 및 에이전트 루프 비용 최적화와 [[agentic-workflow]] 개념을 종합적으로 연계하여 정리.
- **업데이트된 페이지**:
    - [[index.md]] (System) : '종합 (Synthesis)' 카테고리에 신규 추가 및 통계(총 페이지 수 59개) 갱신.
- **잔여 문제**: 없음.

---

## [2026-06-08] ingest | 신규 소스 3건 인제스트 완료
- **인제스트 내용**:
    - [[garry-tan-gstack]] : YC CEO Garry Tan의 gstack 소개 및 AI 소프트웨어 공장 워크플로우 분석.
    - [[opencode-review-1]] : 오픈소스 AI 코딩 에이전트 OpenCode의 특징과 Anthropic 차단 사건에 관한 설치 가이드 요약.
    - [[oh-my-opencode-review]] : oh-my-opencode의 Sisyphus 오케스트레이터 및 멀티 에이전트 병렬 아키텍처 분석.
- **잔여 문제**: 없음.

---

## [2026-06-08] lint | 위키 건강 검진 및 자동 수정 완료
- **건강 점수**: 24 → 90 (🟢 건강)
- **발견 / 수정**: 깨진링크 0/0 (파서 예외 처리), Frontmatter 2/2 수정, 인덱스 불일치 2/2 수정 (연쇄 오류 해결)
- **잔여 문제**: 7건 (stale_content 3건, knowledge_gaps 4건)

---

## [2026-06-08] refine | choi-openai-threads-2026-06 전체 정리 업데이트
- **작업 내용**:
    - [[choi-openai-threads-2026-06]] 문서에 누락되어 있던 6월 첫째 주 스레드 정보(1번~45번 전체 항목)를 원본 소스에서 발췌하여 상세 분석과 함께 전면 갱신 및 복원 완료.
    - **연결 고리 강화**: 메타 보안사고와 [[harness-engineering]]의 연계, 그리고 팩토리 라우터 및 Perplexity 분산 추론과의 아키텍처적 연결성 명시.
- **잔여 문제**: 없음.

---

## [2026-06-08] lint | 위키 건강 검진 및 HIGH 등급 수정
- **건강 점수**: 54 → 94 (🟢 건강)
- **발견 / 수정**: 깨진링크 1/1, Frontmatter 2/2, 인덱스 불일치 1/1, 고아 페이지 2/0 (보류)
- **잔여 문제**: 2건 (고아 페이지)

---

## [2026-06-08] ingest | 신규 소스 4건 인제스트 완료

- **작업 내용**:
    - **신규 소스 요약 생성**:
        - [[gstack-installation-agy]] : Antigravity CLI 환경에서의 gstack 설치 및 팀 모드 설정 가이드.
        - [[gstack-gsd-superpowers-integration]] : gstack(전략), gsd(실행), superpowers(방법론)를 연계한 자율형 프로젝트 관리 파이프라인 가이드.
        - [[choi-openai-threads-2026-06]] : 2026년 6월 첫째 주 AI 업계의 동향(오픈AI Codex 업데이트, MS Scout/Solara, 구글 AI Studio Workspace 연동 등) 동향 요약.
    - **기존 소스 업데이트**:
        - [[ai-development-timeline]] : 중복 및 한국어 번역 버전인 `AI 주요 개념 발전 타임라인.md` 소스 경로를 추가하고 메타데이터 업데이트.
- **잔여 문제**: 없음.

---

## [2026-05-20] refine | GEMINI.md 지식 거버넌스 3대 원칙 대원칙 헌법 정제

- **작업 내용**:
    - **지식 거버넌스 명문화**: 고유 도메인이 특정 프레임워크나 AI 맥락에 맞춰 무리하게 축소·왜곡(아전인수식 해석)되지 않도록 통제하는 **'지식 거버넌스 3대 원칙'**을 운영 헌법인 `GEMINI.md`에 공식 추가함.
    - **헌법 대원칙 튜닝**: 사용자의 헌법적 통찰에 따라, 구체적인 기술 스택 사례(DevOps, SRE, Harness Engineering 등)를 모두 걷어내고, 어떠한 지식 분야에도 보편적으로 적용 가능한 **추상적 대원칙 수준**으로 헌법을 세련되게 정제함.
- **잔여 문제**: 없음.

---

## [2026-05-20] lint | 데브옵스(DevOps) 개념 지식 리팩토링 및 분리

- **건강 점수**: 100 (🟢 건강)
- **작업 내용**:
    - **개념 정의 복원**: 사용자의 지적에 따라 `[[devops]]` 개념 문서를 전면 수정. 하네스의 하위 호환 구조로만 단순화되어 있던 내용을 전통적인 클라우드 엔지니어링, SRE, 서버 인프라, IaC, CI/CD 영역 중심으로 복원함.
    - **패러다임 분리**: 하네스 엔지니어링(AI 에이전트 통치 거버넌스)과 데브옵스(인프라 신뢰성 자동화)를 수직 종속 관계가 아닌, 독립적이면서 대조되는 두 개의 전문 패러다임으로 명확히 정립 및 분리.
- **잔여 문제**: 없음.
