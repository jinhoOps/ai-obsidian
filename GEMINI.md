...
에이전트는 다음 네 가지 핵심 명령을 통해 지식 베이스를 관리합니다. 모든 작업 결과는 `log.md`에 기록됩니다.

### 1. Compile (컴파일)
- **목적**: `raw/` 폴더의 신규 정보를 분석하여 `wiki/`에 정제된 노트로 반영.
- **절차**: 
  1. `raw/` 파일 읽기 및 독립적이고 단일한 주제 단위(원자성)로 핵심 개념 분리 및 추출.
  2. `wiki/`에 새로운 요약/엔티티 페이지 생성 또는 기존 페이지 업데이트.
  3. 관련 노트 간 위키 링크(`[[...]]`) 삽입.
  4. `index.md` 및 `log.md` 업데이트.
  5. 처리가 완료된 원본은 `raw/archive/`로 이동.

### 2. Query (쿼리)
- **목적**: 사용자의 질문에 대해 지식 베이스를 바탕으로 종합적인 답변 제공.
- **절차**:
  1. `index.md` 및 관련 `wiki/` 노드를 우선 탐색.
  2. 출처(Citation)를 포함한 답변 생성.
  3. 필요시 새로운 합성 지식을 `wiki/` 노드로 저장.
...
- **명령어**: `compile`
1. **Analyze (분석)**: `raw/` 폴더 내의 신규 및 변경된 파일을 스캔.
2. **Filter (필터링)**: 이미 반영된 파일 제외.
3. **Refine (정제)**: `wiki/` 하위 토픽 정리 및 인덱스 업데이트.
4. **Archive & Track (보관 및 추적)**: 완료된 파일 이동 및 상태 추적.
5. **Index (인덱스 갱신)**: `index.md` 및 `_master-index.md` 최신화.
