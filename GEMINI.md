...
3. **Refine (정제)**: `content/wiki/` 하위 토픽 정리 및 인덱스 업데이트.
   - **파일명 최적화 가이드라인 (Naming Convention)**:
     - **포맷**: `[UID/ID]_[핵심키워드(15~30자 이내)].md` (날짜 접두사 제거, 원본 보존)
     - **속성 이관**: 원본 문서의 발생/작성 날짜는 Frontmatter의 `original_date: YYYY-MM-DD` 속성에 저장합니다.
     - **원본 보존**: 문서 내용의 H1 제목은 원본 전체 제목을 유지하여 정보 손실을 방지합니다.
